...
class Simplify:
    ...
    @staticmethod
    def douglas_peucker(
        qgs_in_features, tolerance, validate_structure=False, feedback=None
    ):
    ...
    def find_farthest_point(
        ...
        if last - first >= 2:
            ...
            distances = [
                qgs_geom_engine.distance(qgs_points[i]) for i in range(first + 1, last)
            ]
    ...
    def reduce(self):
        ...
        qgs_features_out = [
            gs_feature.get_qgs_feature() for gs_feature in self.gs_features
        ]
    ...
    def validate_constraints(self, sim_geom, first, last):
        ...
        if constraints_valid and len(qgs_geoms_with_others) >= 1:
            constraints_valid = GeoSimUtil.validate_intersection(
                qgs_geoms_with_others, qgs_geom_new_subline
            )
        ...
        if constraints_valid and len(qgs_geoms_with_others) >= 1:
            ...
            qgs_ls_old_subline.addVertex(
                qgs_points[0]
            )  # Close the line with the start point
            ...
            if qgs_geom_polygonize.isSimple():
                constraints_valid = GeoSimUtil.validate_sidedness(
                    qgs_geoms_with_others, qgs_geom_polygonize
                )
            else:
    ...
    def init_process_line_stack(is_line_closed, qgs_points):
        ...
        if is_line_closed:
            ...
            if last_index >= 4:
                ...
                mid_index = lst_distance.index(
                    max(lst_distance)
                )  # Most distant vertex position

                (farthest_index_a, farthest_dist_a) = Simplify.find_farthest_point(
                    qgs_points, 0, mid_index
                )
                (farthest_index_b, farthest_dist_b) = Simplify.find_farthest_point(
                    qgs_points, mid_index, last_index
                )
                if farthest_dist_a > 0.0:
                    stack.append((0, farthest_index_a))
                    stack.append((farthest_index_a, mid_index))
                if farthest_dist_b > 0.0:
    ...
    def process_line(self, sim_geom):
        ...
        while stack:
            ...
            if first + 1 < last:  # The segment to check has only 2 points
                (farthest_index, farthest_dist) = Simplify.find_farthest_point(
                    qgs_points, first, last
                )
                if farthest_dist <= self.tolerance:
                    if self.validate_constraints(sim_geom, first, last):
                        ...
                        sim_geom.is_simplest = (
                            False  # The line string is not at its simplest form
                        )
